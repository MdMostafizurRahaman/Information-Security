...
void F(uint64t *block, int block_number)
{
  ...
  for (int i = 16; i < number_of_steps; i++) W[i] = W[i - 16] + W[i - 7] + Sigma0(W[i - 15]) + Sigma1(W[i - 2]);
  for (int i = 0; i < number_of_steps; i++) round(i);
  for (int i = 0; i < 8; i++) initial_vector[i] = IV[i] + initial_vector[i];
  for (int i = 0; i < 8; i++) IV[i] = initial_vector[i];
}
...
int main(void)
{
  ...
  freopen("HASH_1320.txt", "w", stdout);
  ...
  SHA_512(message);
  //cout << endl << "Message digest:- " << endl;
  for (int i = 0; i < 8; i++) printf("%llX ",initial_vector[i]);
  cout << endl << endl;
